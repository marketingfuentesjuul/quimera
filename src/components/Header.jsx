    ...
    <header className={`${isContactPage ? 'absolute' : 'fixed'} w-full z-50 transition-all duration-500 flex justify-center ${
        ...
        <nav className="w-full flex justify-between items-center font-['Inter'] text-sm font-medium tracking-tight">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              <a
                href="https://www.linkedin.com/company/110639034/admin/dashboard/"
                target="_blank"
      ...
      >
        <div className="flex flex-col gap-4 text-center">
          ...
          {/* Mobile CTA Button and LinkedIn Icon */}
          <div className="flex items-center justify-center gap-4 mt-6 pt-4 border-t border-slate-100/10">
            {ctaButton.isRoute ? (
              <Link
                viewTransition
                to={ctaButton.path}
                onClick={() => setMobileMenuOpen(false)}
                className="btn btn-primary text-xs font-bold tracking-wider"
                style={{ textTransform: 'none', padding: '0.65rem 1.5rem', borderRadius: 'var(--radius-full)' }}
              >
                {ctaButton.text}
              </Link>
            ) : (
              <a
                href={ctaButton.path}
                onClick={(e) => {
                  if (ctaButton.isScroll) handleScrollTo(e, ctaButton.path);
                  setMobileMenuOpen(false);
                }}
                className="btn btn-primary text-xs font-bold tracking-wider"
                style={{ textTransform: 'none', padding: '0.65rem 1.5rem', borderRadius: 'var(--radius-full)' }}
